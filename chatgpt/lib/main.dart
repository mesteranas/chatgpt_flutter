      ...
      return AlertDialog(title: Text(_("about")+" "+App.name),content:Center(child:Column(children: [
        ...
        ,child: Column(children: [
          ...
          ElevatedButton(onPressed: () async{
            setState(() {
              result=_("please wait chatgpt is writing a message");
            });
            var r=await http.get(Uri.parse("https://chatgpt.apinepdev.workers.dev/?question=" + message.text.replaceAll(" ", "%22")));
            var resultFromChatgpt=jsonDecode(r.body);
            setState(() {
              result=resultFromChatgpt["answer"];
            });
          }, child: Text(_("send"))),
          Text(result)
    ])),)));
